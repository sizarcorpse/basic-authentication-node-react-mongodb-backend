...
const isAuthenticatedUser = require("./isAuthenticatedUser");

router.get("/", isAuthenticatedUser, (req, res) => {
  res.send({ Login: "Seccess", username: req.user.username });
});
...
router.post("/signup", async (req, res, next) => {
  ...
  if (error) {
    ...
    // check username exits on database ? forword : send error
